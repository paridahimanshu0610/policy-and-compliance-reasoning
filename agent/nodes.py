...
def intake_node(state: AgentState) -> dict:
    ...
    what we already know. List fields are unioned; scalar fields are only
    overwritten when the new message actually states them."""
    llm = get_chat_model("intake").with_structured_output(ExtractedFacts)

    known_so_far = state.get("known_fields", {})
    context = (
        f"Facts already known: {known_so_far}\n\n"
        f"Latest message from the user: {state['raw_query']}"
    )
    ...
    for field, value in extracted.model_dump().items():
        ...
        if isinstance(value, list):
            existing = set(updated.get(field, []))
            updated[field] = sorted(existing.union(value))
        else:
            updated[field] = value  # scalar: latest stated value wins
...
def ambiguity_node(state: AgentState) -> dict:
    ...
    hits = vector_search(state["raw_query"], top_k=10)
    if not hits:
    ...
    result = llm.invoke([
        ...
        HumanMessage(content=f"User question: {state['raw_query']}\n\nCandidate topics found:\n{summary_of_clusters}"),
    ])
...
def retrieve_node(state: AgentState) -> dict:
    ...
    filter_conditions = {
        ...
        if field in _FILTERABLE_FIELDS and value not in (None, [], "")
    }

    query_text = state.get("needs_more_search") or state["raw_query"]
